...
- `military` — war, army (Bellum, Miles, Dux, Gladius, Hasta, Scutum, Legio, Hostis, Victoria, Proelium, Vulnus, Exercitus, Centurio)
...
**This file has the opposite lifecycle from the vocab files.** The vocab
files are append-and-edit-only; `sentences.json` is **wiped and regenerated
wholesale** on every refresh. The point of the feature is testing
translation ability, not sentence recall, so the practice set must not
accumulate into a fixed, memorizable bank. Regeneration should vary
...
the thing that gets memorized. The `generated` date records when the
