...
singular active-voice verbs only, at most one adjective per sentence) on
top of the shared guardrails every tier follows.
...
four paragraphs are unrelated to one another: a batch of 20 is four
separate 5-sentence stories, one per tier. Selecting several tiers at once
plays their paragraphs back to back (warm-up, then easy, then
intermediate, then challenging) instead of interleaving them.
...
guardrails loosen as the user's Latin improves. All four difficulty tiers
...
banned, sequencing words included. `warm-up` additionally restricts nouns
to 1st/2nd declension, verbs to singular active voice, and caps each
sentence at one adjective — see CLAUDE.md's "Difficulty tiers" for the
full rubric.
